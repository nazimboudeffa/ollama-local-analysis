...
## 9. Passage à l'IA et sauvegarde du rapport

Le YAML est sérialisé (`yaml.dump`) et injecté dans le prompt (cf. [prompt_ia.md](prompt_ia.md)). La réponse du modèle est **Markdown brut** affiché tel quel puis **sauvegardé** dans `rapports_ia/` :

```python
ai_result = ask_ai(yaml.dump(signal_yaml, sort_keys=False, allow_unicode=True))
if ai_result:
    stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    out_path = os.path.join("rapports_ia", f"{stamp}_{MODEL}_{SYMBOL}.md")
    # ex : rapports_ia/20260802_002659_gemma4_GBPUSD.md
```

Format du nom : `AAAAMMJJ_HHMMSS_<modèle>_<paire>.md` (date + IA utilisée + paire analysée).
...
| `rapports_ia/` | Rapports IA enregistrés (date_modèle_paire.md) |
